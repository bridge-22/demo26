...
conf t
hostname hq-rtr 
ip domain-name au-team.irpo write memory
exit
write memory
exit
  ```
...
systemctl restart network
echo nameserver 8.8.8.8 > /etc/resolv.conf
timedatectl set-timezone Asia/Yekaterinuburg
timedatectl
```
...
apt-get update; apt-get reinstall tzdata; timedatectl set-timezone Asia/Yekaterinburg; timedatectl
```
