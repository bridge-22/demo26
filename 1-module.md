...
systemctl restart network
useradd  -u 2026 && echo "P@ssw0rd" | passwd --stdin sshuser && gpasswd -a "sshuser" wheel && echo '%wheel ALL=(ALL:ALL) NOPASSWD: ALL' > /etc/sudoers.d/99-wheel-nopasswd
...
systemctl restart network
useradd sshuser -u 2026 && echo "P@ssw0rd" | passwd --stdin sshuser && gpasswd -a "sshuser" wheel && echo '%wheel ALL=(ALL:ALL) NOPASSWD: ALL' > /etc/sudoers.d/99-wheel-nopasswd
echo -e "Port 2026\nMaxAuthTries 2\nPasswordAuthentication yes\nAllowUsers sshuser\nBanner /etc/openssh/banner" > /etc/openssh/sshd_config
echo -e "Authorized access only!" > /etc/openssh/banner
echo nameserver 8.8.8.8 > /etc/resolv.conf
timedatectl set-timezone Asia/Yekaterinburg
